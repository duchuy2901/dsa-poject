...
public class CreateCollision {
	...
	private void checkCollisionOnTopOfEntiy() {
		...
		int topColRight = (int) ((entity.worldX + entity.solidArea.x + entity.solidArea.width
				- entity.solidArea.width / 4) / gp.tileSize);
		int rowCol = (int) ((entity.worldY + ySpeed) / gp.tileSize);
		int topNumLeft = gp.currentSceneLevel.tileM.mapTileNum[topColLeft][rowCol];
		int topNumRight = gp.currentSceneLevel.tileM.mapTileNum[topColRight][rowCol];
//	
		if (gp.currentSceneLevel.tileM.tile[topNumLeft].collision) {
			ySpeed = 0;
			ySpeed += acceleration;
	...
	public boolean canMoveOnRightSide(float xVelocity) {
		...
		int topRight = gp.currentSceneLevel.tileM.mapTileNum[entityRightCol][entityTopRow];
		int entityBottomRow = (int) (entityBottomWorldY - gp.tileSize / 2) / gp.tileSize;
		int downRight = gp.currentSceneLevel.tileM.mapTileNum[entityRightCol][entityBottomRow];
		// kiem tra ben duoi va ben tren nhan vat o ben phai khi di chuyen co va cham
		// voi ai k
		// neu co ==> stop di chuyen
		if (gp.currentSceneLevel.tileM.tile[topRight].collision
				|| gp.currentSceneLevel.tileM.tile[downRight].collision) {
	...
	public boolean canMoveOnLeftSide(float xVelocity) {
		...
		entityTopRow = entityTopWorldY / gp.tileSize;
		int topleft = gp.currentSceneLevel.tileM.mapTileNum[entityLeftCol][entityTopRow];
		int entityBottomRow1 = (int) (entityBottomWorldY - gp.tileSize / 2) / gp.tileSize;
		int downLeft = gp.currentSceneLevel.tileM.mapTileNum[entityLeftCol][entityBottomRow1];
		if (gp.currentSceneLevel.tileM.tile[topleft].collision || gp.currentSceneLevel.tileM.tile[downLeft].collision) {
			entity.preventMove = true;
			return false;
		}
		return true;
	...
	private void LogicGravityGround() {
		...
		int middle = gp.currentSceneLevel.tileM.mapTileNum[entityBottomCol][entityBottomRow];
		...
		int left = gp.currentSceneLevel.tileM.mapTileNum[entityBottomLeftCol][entityBottomRow];
		...
		int right = gp.currentSceneLevel.tileM.mapTileNum[entityBottomRightCol][entityBottomRow];
		// check neu isground 1 trong 3 ---
		if (gp.currentSceneLevel.tileM.tile[middle].isGround || gp.currentSceneLevel.tileM.tile[left].isGround
				|| gp.currentSceneLevel.tileM.tile[right].isGround) {
	...
	private void flipWhenNotHaveBlockInFront(int entityBottomRow, int left, int right) {
		...
		if (entity.isEnemy) {
			...
			if (gp.currentSceneLevel.tileM.tile[left].isGround) {
				...
				left = gp.currentSceneLevel.tileM.mapTileNum[entityBottomLeftCol][entityBottomRow];
				if (!gp.currentSceneLevel.tileM.tile[left].isGround) {
					entity.faceDirection = 1;
			...
			if (gp.currentSceneLevel.tileM.tile[right].isGround) {
				...
				right = gp.currentSceneLevel.tileM.mapTileNum[entityBottomRightCol][entityBottomRow];
				if (!gp.currentSceneLevel.tileM.tile[right].isGround) {
					entity.faceDirection = -1;
