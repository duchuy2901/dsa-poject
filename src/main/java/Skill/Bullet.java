...
public class Bullet {
	...
	public Bullet(GamePanel _gp, Player _player, Player _enemiesPlayer, int _worldX, int _worldY, int _faceDirection) {
		this.gp = _gp;
		this.player = _player;
		this.enemiePlayer = _enemiesPlayer;
		this.worldX = _worldX;
		this.worldY = _worldY;
		this.defaultworldX = _worldX;
		this.defaultworldY = _worldY;
	...
	public void draw(Graphics2D g2) {
		...
		if (gp.mainGameOnlineScene != null) {
			...
			int screenX = worldX - mainPlayer.worldX + mainPlayer.screenX;
			int screenY = worldY - mainPlayer.worldY + mainPlayer.screenY;
...
//			}
		...
		if (worldX + width >= player.worldX - player.screenX && worldX - width <= player.worldX + player.screenX
	...
	private void checkCollisionWithEnemy() {
		if (enemies != null) {
			for (int i = 0; i < enemies.size(); i++) {
				...
				if (worldX + solidArea.x <= rect2X + rect2Width && worldX + solidArea.x + solidArea.width >= rect2X
		...
		if (worldX + solidArea.x <= rect2X + rect2Width && worldX + solidArea.x + solidArea.width >= rect2X
				&& worldY + solidArea.y <= rect2Y + rect2Height && worldY + solidArea.y + solidArea.height >= rect2Y) {
			// xoa khi ra khoi the gioi
			// isActive = false;
			speed = 0;
			if (!isCollision) {
				isCollision = true;
				if (enemy.canAttacked)
					enemy.getDamaged(faceDirection);
				playSound("boom");
	...
	private void setUpSound() {
		try {
			// AudioInputStream
			// audioInputStream=AudioSystem.getAudioInputStream(getClass().getResource(null))
			AudioInputStream ais = AudioSystem
					.getAudioInputStream(getClass().getResource("/music/laser-gun-81720 (mp3cut.net) (1).wav"));
			shootClip = AudioSystem.getClip();
			shootClip.open(ais);
			AudioInputStream ais2 = AudioSystem
					.getAudioInputStream(getClass().getResource("/music/GSCC5QS-futuristic-gun (mp3cut.net).wav"));
